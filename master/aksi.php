...
$first = Quartiles($vals, 0.25);
$second = Quartiles($vals, 0.5);
$third = Quartiles($vals, 0.75);
...
////////////////////////////////////////////////////////
$status=array();
foreach($data as $id_datas=>$dt_atribut){
...
for($x=1;$x<=count($parameter[1]);$x++){
    for($y=2;$y<=$jml_kriteria;$y++){
        for($z=1;$z<=count($parameter[$y]);$z++){
            $y1=$y-1;
            if(!isset($isi[$x][$y1][$z])){
                ...
                $isi[$x][$y1][$z]=$isi[$x][$y1][$z]/$status[$z];
            }
...
$training=array('fazjar',2,1,2,2,2,1,1,1);
...
echo $jumlaht1+$jumlaht2+$jumlaht3+$jumlaht4;
?>